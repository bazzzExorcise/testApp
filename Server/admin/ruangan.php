...
$row = mysqli_query($conn, "SELECT * FROM account WHERE status = 'ruangan'");
?>
...
<div class="relative w-full flex flex-col items-center max-w-sm max-h-screen overflow-auto">
  <div class="w-full p-4">
    ...
    <h1 class="text-xl font-extrabold">Daftar ruangan</h1>
    <p>daftar ruangan ujian</p>
    <form action="../system/search-ruangan.php" class="relative mt-2 w-full overflow-hidden rounded-lg border-2 border-black bg-yellow-400">
      <input type="text" class="search font-extrabold lowercase focus:outline-none w-full bg-transparent px-3 py-2 placeholder:font-extrabold placeholder:text-black" placeholder="cari ruangan" name="search" />
    </form>
    <button class="w-full button-pop rounded-lg border-2 border-black bg-green-400 px-3 py-2 font-extrabold">Tambah ruangan</button>
  ...
  <div class="font-extrabold add-wall opacity-0 -z-30 fixed bg-white/50 backdrop-blur-sm duration-500 top-0 left-0 h-screen w-full grid place-items-center">
    <div class="p-4 max-w-xs bg-sky-300 w-full rounded-md flex flex-col items-center justify-center gap-2 border-2 border-black">
      ...
      <h1 class="text-2xl text-center">Tambah ruangan</h1>
      <form class="add-data w-full flex flex-col gap-2" action="../system/add-ruangan.php" method="post">
        <div class="w-full alert-box"></div>
        <input type="text" name="nama" class="start border-2 border-black input-add rounded-lg py-2 px-3 placeholder:text-black" placeholder="nama ruangan" id="">
  ...
  <div class="font-extrabold update-wall opacity-0 -z-30 fixed bg-white/50 backdrop-blur-sm duration-500 top-0 left-0 h-screen w-full grid place-items-center">
    <div class="p-4 max-w-xs bg-sky-300 w-full rounded-md flex flex-col items-center justify-center gap-2 border-2 border-black">
      <div class="w-full flex justify-end end-pop"><ion-icon name="close"></ion-icon></div>
      <h1 class="text-2xl text-center">Edit Data ruangan</h1>
      <form class="update-data w-full flex flex-col gap-2" action="../system/update-ruangan.php" method="post">
        <div class="w-full update-box"></div>
        <input type="text" name="edit-nama" class="start border-2 border-black input-update rounded-lg py-2 px-3 placeholder:text-black" placeholder="nama ruangan" id="">
        <input type="hidden" name="id" class="start border-2 border-black input-update rounded-lg py-2 px-3 placeholder:text-black" placeholder="password ruangan" id="">
        <button type="submit" class="bg-green-400 rounded-lg w-full py-2 text-center border-2 border-black">Buat</button>
      </form>
    </div>
  </div>
  <script>
    ...
    function getData() {
      $.get("../system/ruangan.php", function(data) {
        $(".content").html(data);
